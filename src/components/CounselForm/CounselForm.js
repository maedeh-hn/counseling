import classes from './CounselForm.module.css';
...
class CounselForm extends Component{
state={
    controls:{
        name:{
            label:'    نام',
    ...
    const  data={
            ...
            tahsil:this.state.controls.tahsil.value
    ...
    imgUploadHandler = () => {
        axios.post('https://jsonplaceholder.typicode.com/posts', this.state.selectedProfil)
      }
      imgFileChangedHandler = event => {
        this.setState({ selectedProfil: event.target.files[0] })
      }
      uploadCardHandler = () => {
        axios.post('https://jsonplaceholder.typicode.com/posts', this.state.selectedCard)
      }
      CardfileChangedHandler = event => {
        this.setState({ selectedCard: event.target.files[0] })
      }
    ...
    render(){
        ...
        let form=(
            <form  onSubmit={this.submitHandler}>
               ...
               <div><label>عکس پروفایل</label> <input  type="file" className={classes.upload} onChange={this. imgFileChangedHandler}/>
                <button onClick={this. imgUploadHandler}>آپلود کنید!</button></div>
                <div><label>عکس کارت ملی</label> <input  type="file" className={classes.upload} onChange={this.CardfileChangedHandler}/>
                <button onClick={this. uploadCardHandler}>آپلود کنید!</button></div>
